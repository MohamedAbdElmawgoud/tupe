...
export class CreateCompPage implements OnInit {
  ...
  async getCompaign() {
    ...
    this.firebaseService.getDocumentsCampaigns().subscribe(docs => {
      let docsId = docs.map(ele => ele.payload.doc.id)
      docsId.forEach(ele=>{
        this.firebaseService.getDocumentElement(ele).subscribe(doc=>{
          console.log(doc);
